...
use App\Services\SelectPopulationHelper;
...
class MainController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->pn = $pn;
        $this->SelectPopulationHelper = new SelectPopulationHelper();
    }
    ...
    public function calcarea_level(
        $id,
        $lvl,
        GenetixDataGenerator $gtx,
        CrossingData $cross,
        MutationData $mutation,
        BigMutatorData $bigmutation,
        PowerBigMutator $powermutation,
        Generation0Helper $gen0,
        $dId = null
    ) {
        ...
        if ($lvl <= 0) {
            ...
            $pattern = $this->SelectPopulationHelper->getPatternFromPowerUsed($randomDoing, $dataBest, $power, $gtx);
            ...
            $orderDesc = $this->SelectPopulationHelper->getOrderForAvgData($randomDoing);

            $ids = Accuratecalc::where("area_id", $id)->take(50)->orderBy($orderDesc[0],  $orderDesc[1])->get();
            $selected = [];
            if ($ids) {
                foreach ($ids as $ac) {
                    $selected[] = $ac->calc_id;
            ...
            $this->SelectPopulationHelper->setPowerMutation($randomDoing, $powermutation, $this->usePowerMutator);
        ...
        while ($repeatQ < 20 && $nrPop < $this->maxPopulation && $maxQ < $maxPoints) {
            ...
            if (in_array($nrPop, $halfStep)) {
                ...
                if ($halfreso >= $minimumCalc2) {
                    Calculation::create([
                        "result" => "Wynik pośredni ",
                        "data" => json_encode($res[0]['area']),
                        "area_id" => $id,
                        "level" => $lvl + 1,
                        "obtainedresult" => $halfreso,
                        "typecalc" => 30,
                        "population" => $nrPop
                    ]);
                }
        ...
        if ($result2  > $minimumCalc) {
            ...
            $cred = Calculation::create([
                "result" => $name,
                "data" => json_encode($res[0]['area']),
                "area_id" => $id,
                "level" => $lvl + 1,
                "obtainedresult" => $result2,
                "usedmod" => json_encode($usedmodify),
                "typecalc" => $randomDoing,
                "population" => $nrPop,
                "info" => json_encode($info),
                "progress" => $last / $first,
                "start" => $first / $maxPoints,
                "progcalc" => $pgc
            ]);
            ...
            if ($lvlReso[0] > 0) {
                ...
                $calco = Calculation::create([
                    "result" => "Spadocorniarz z " . ($lvl + 1) . " na level " . $lvlReso[0] . " (" . $randomDoing . ") " . $timemsg,
                    "data" => json_encode($res[0]['area']),
                    "area_id" => $id,
                    "level" => $lvlReso[0],
                    "obtainedresult" => $result2,
